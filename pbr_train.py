...
def pbr_training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    print ("iterative interval: ", opt.iter_interval)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration < opt.warmup_iterations:
            ...
            if iteration % (2*opt.iter_interval) < opt.iter_interval:
                # fw
                ...
                try:
                    gt_mask = viewpoint_cam.gt_normal_mask.cuda()
        ...
        # dist_loss = 0.0 # no dist loss for now

        total_loss = loss + dist_loss + normal_loss

        # try:
        #     rend_normal.retain_grad()
        # except:
        #     pass
        ...
        with torch.no_grad():
            ...
            if iteration < opt.warmup_iterations:
            ...
            elif iteration % (2*opt.iter_interval) < opt.iter_interval:
                training_report(tb_writer, grad_dict, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
                                ...
                                (cubemap, pipe, background, brdf_lut, False, dataset.metallic)
                                )
            else:
                training_report(tb_writer, grad_dict, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
