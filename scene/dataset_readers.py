...
def fetchPly_woColors(path: str) -> BasicPointCloud:
    plydata = PlyData.read(path)
    vertices = plydata["vertex"]
    positions = np.vstack([vertices["x"], vertices["y"], vertices["z"]]).T
    normals = np.vstack([vertices["nx"], vertices["ny"], vertices["nz"]]).T
    colors = np.random.rand(*normals.shape).astype(normals.dtype)
    return BasicPointCloud(points=positions, colors=colors, normals=normals)
...
def readCamerasFromTransforms(
    path: str, transformsfile: str, white_background: bool, extension: str = ".png" , load_normal: bool = False
) -> List[CameraInfo]:
    cam_infos = []
...
def readNerfSyntheticInfo(
    path: str, white_background: bool, eval: bool, extension: str = ".png" , load_normal: bool = False
) -> SceneInfo:
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, extension, load_normal=load_normal
    )
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        path, "transforms_test.json", white_background, extension, load_normal=load_normal
    )
    ...
    try:
        # pcd = fetchPly(ply_path)
        pcd = fetchPly_woColors(ply_path) #no rgb pointcloud for teapot
    except:
        pcd = None
