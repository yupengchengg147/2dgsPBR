...
def render_set(model_path, name, iteration, views, gaussians, cubemap,  pipeline, background, canonical_rays):
...
def render_sets(dataset : ModelParams, chkp_path: str, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", first_iter, scene.getTrainCameras(), gaussians, cubemap,  pipeline, background, canonical_rays)

        if not skip_test:
             render_set(dataset.model_path, "test", first_iter, scene.getTestCameras(), gaussians, cubemap, pipeline, background, canonical_rays)
