# train with gt mask, which can mask out bg 
import os
import sys
import torch
...
def pbr_training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        #loss with gt mask
        gt_mask = viewpoint_cam.gt_normal_mask.cuda() #[1,h,w]

        image = torch.where(gt_mask, image, bg[:, None, None])
        rend_normal = torch.where(gt_mask, rend_normal, bg[:, None, None])
        normal_from_d = torch.where(gt_mask, normal_from_d, bg[:, None, None])
        ...
        with torch.no_grad():
            ...
            ema_dist_for_log = 0.4 * (dist).mean().item() + 0.6 * ema_dist_for_log
